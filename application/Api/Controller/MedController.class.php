...
class MedController extends MedbaseController{
     ...
     */
     public function audio_info(){
         ...
         $audio_info['audio_cover'] = $this->handler_audio_url( $audio_info['audio_cover'],"audio");
            ...
            foreach($loft_list as $key=>$value){
                ...
                foreach($item_list as $k=>$v){
                    ...
                    if($value['loft_type']=='adv'){
                        ...
                        $result[$i]['adv_img'] = $this->handler_img_url($v['adv_img']);
                    }else{
                        $result[$i]['loft_type'] = $value['loft_type'];
                        $result[$i]['loft_display'] = $value['loft_display'];
                        if($v['album_ids']){
    ...
    public function layout_index_special(){
        ...
        //点播次数排序权重
         ...
         $album_list = $audio_album->field('id album_id,album_name,album_cover,album_like_count')->select();
         foreach($album_list as $key=>$value){
        ...
        $adv_list = $med_loft_item->where($where)->select();
        $i=0;
        foreach($adv_list as $key=>$value){
            $result[$i]['loft_type'] = 'adv';
            $result[$i]['loft_display'] = 1;
            $result[$i]['adv_img'] = $this->handler_img_url($value['adv_img']);
            $i++;
        }
        ...
        $home2_loft = array(
            'loft_type'=>'home2',
            'loft_display'=>1,
            'item_list'=>array_slice($album_list_sort,4,5)
        );
    ...
    public function audio_share_count_increase(){
        ...
        if($album_id){
            ...
            if($album_history_model->where(array("user_id"=>$user_id,"difficulty_degree"=>$difficulty_degree))->count()){
                ...
                $rs = $album_history_model->where(array("user_id"=>$user_id,"difficulty_degree"=>$audio_info['audio_difficulty']))->save($difficulty_history_data);
            }else{
                $difficulty_history_data = array(
                    'user_id'=>$user_id,
                    'difficulty_degree'=>$audio_info['audio_difficulty'],
